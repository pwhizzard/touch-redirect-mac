...
/// Handles multi-touch gesture recognition and active-device arbitration
class GestureEngine {
    ...
    private(set) var activeDeviceIdentity: TouchDeviceIdentity?

    /// Per-device mappers (keyed by persistence key). Falls back to global mapper.
    private var deviceMappers: [String: Mapper] = [:]
    ...
    // MARK: - Device Mapper Management

    /// Register or retrieve a per-device mapper
    func mapper(for identity: TouchDeviceIdentity, profile: TouchDeviceProfile) -> Mapper {
        let key = identity.persistenceKey
        if let existing = deviceMappers[key] {
            return existing
        }
        let newMapper = Mapper(profile: profile, deviceIdentity: identity)
        deviceMappers[key] = newMapper
        log("Created per-device mapper for \(profile.displayName) (\(identity))")
        return newMapper
    }

    /// Remove mapper when a device disconnects
    func removeMapper(for identity: TouchDeviceIdentity) {
        deviceMappers.removeValue(forKey: identity.persistenceKey)
    }

    /// Remove all per-device mappers and reset active device state.
    /// Call when calibration is globally reset.
    func resetAllMappers() {
        let count = deviceMappers.count
        deviceMappers.removeAll()
        activeDeviceIdentity = nil
        if count > 0 {
            log("Cleared \(count) per-device mapper(s) on reset")
        }
    }

    /// Clean up stale per-device mappers and active device state.
    /// Call when the set of connected devices changes.
    func cleanupDisconnectedDevices(connectedIdentities: Set<TouchDeviceIdentity>) {
        // Remove mappers for devices no longer connected
        let staleKeys = deviceMappers.keys.filter { key in
            !connectedIdentities.contains(where: { $0.persistenceKey == key })
        }
        for key in staleKeys {
            deviceMappers.removeValue(forKey: key)
            log("Cleaned up stale mapper for device key: \(key)")
        }

        // If the active device disconnected, release it
        if let active = activeDeviceIdentity, !connectedIdentities.contains(active) {
            log("Device arbitration: active device disconnected (\(active)), releasing")
            activeDeviceIdentity = nil
        }
    }
    ...
    /// Applies active-device arbitration before gesture processing.
    func processTouchReport(_ report: TouchReport) {
        guard config.enabled else {
            return
        }

        // --- Active Device Arbitration (first-touch-wins) ---
        if let sourceIdentity = report.deviceIdentity {
            if let currentActive = activeDeviceIdentity {
                if currentActive != sourceIdentity {
                    // Another device is active — ignore this report
                    return
                }
            } else {
                // No device active yet — claim if report has active touches
                let hasActiveTouches = report.touches.contains { $0.tipSwitch }
                if hasActiveTouches {
                    activeDeviceIdentity = sourceIdentity
                    log("Device arbitration: \(sourceIdentity) claimed active control")
                } else {
                    return  // No active touches, nothing to do
                }
            }
        }
        
        // Select the appropriate mapper for this report
        let activeMapper: Mapper
        if let identity = report.deviceIdentity,
           let profileID = report.profileID {
            let profile = TouchDeviceProfile.profile(for: profileID)
            activeMapper = self.mapper(for: identity, profile: profile)
        } else {
            activeMapper = mapper  // Global/legacy fallback
        }
        ...
        for touch in report.touches {
            let screenPos = activeMapper.mapToScreen(touchX: touch.x, touchY: touch.y)
