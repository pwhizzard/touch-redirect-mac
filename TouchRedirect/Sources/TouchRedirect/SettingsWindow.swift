...
struct GeneralSettingsView: View {
    ...
    @State private var connectedDevices: [ConnectedDeviceDescriptor] = []
    ...
    var body: some View {
        Form {
            ...
            // Per-device display binding section is shown when devices are connected
            if !connectedDevices.isEmpty {
                Section {
                    ForEach(connectedDevices) { device in
                        DeviceDisplayBindingRow(
                            device: device,
                    ...
                    if connectedDevices.count > 1 {
                        Text("Each device is automatically paired to its matching display. Override manually if needed.")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    } else {
                        Text("The first device you touch controls the pointer until you lift all fingers.")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                } header: {
        ...
        .onAppear {
            refreshScreenList()
            // Request fresh device list from HIDManager (may already be connected)
            NotificationCenter.default.post(name: .requestDeviceListBroadcast, object: nil)
        }
        .onReceive(NotificationCenter.default.publisher(for: .connectedTouchDevicesChanged)) { notification in
            if let devices = notification.userInfo?["devices"] as? [ConnectedDeviceDescriptor] {
...
// MARK: - Per-Device Display Binding Row

struct DeviceDisplayBindingRow: View {
    let device: ConnectedDeviceDescriptor
    let availableScreens: [(id: CGDirectDisplayID, name: String, resolution: CGSize)]

    @State private var selectedDisplayID: CGDirectDisplayID

    init(device: ConnectedDeviceDescriptor,
         availableScreens: [(id: CGDirectDisplayID, name: String, resolution: CGSize)]) {
        self.device = device
        self.availableScreens = availableScreens
        let saved = Config.shared.boundDisplayID(forDevice: device.id)
        _selectedDisplayID = State(initialValue: CGDirectDisplayID(saved))
    }

    /// The auto-correlated display (best guess from profile keywords)
    private var autoCorrelatedScreen: (id: CGDirectDisplayID, name: String)? {
        let profile = TouchDeviceProfile.profile(for: device.profileID)
        guard !profile.screenNameKeywords.isEmpty else { return nil }
        for screen in availableScreens {
            let upper = screen.name.uppercased()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack {
                ...
                Text(device.profileName)
                    .fontWeight(.medium)
            }

            Picker("Display", selection: $selectedDisplayID) {
                if let auto = autoCorrelatedScreen {
                    Text("Auto (\(auto.name))").tag(CGDirectDisplayID(0))
                } else {
            ...
            .onChange(of: selectedDisplayID) { _, newValue in
                Config.shared.setBoundDisplayID(Int(newValue), forDevice: device.id)
                // Notify runtime to re-resolve display targets
                NotificationCenter.default.post(
                    name: .deviceDisplayBindingsChanged,
                    object: nil
                )
            }
        }
...
struct CalibrationSettingsView: View {
    ...
    var body: some View {
        Form {
            Section {
                ...
                Button("Reset to Defaults") {
                    // Post notification to let AppDelegate handle full reset
                    // (clears config, global mapper, and per-device mapper cache)
                    NotificationCenter.default.post(
            ...
            Section {
                VStack(alignment: .leading) {
                    ...
                    Text("Compensates for devices that reserve edge zones for their own gestures. Higher values push touch closer to screen edges.")
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
            } header: {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .calibrationDidChange)) { _ in
            updateCalibrationInfo()
        }
    }
    
    private func updateCalibrationInfo() {
        // Check for global or any per-device calibration
        let hasGlobal = Config.shared.affineTransform != nil
        let defaults = UserDefaults.standard
        let hasPerDevice = defaults.dictionaryRepresentation().keys.contains(where: {
            $0.hasPrefix("device.") && $0.hasSuffix(".affineTransform")
        })
        hasCalibration = hasGlobal || hasPerDevice
        ...
        if let res = Config.shared.calibrationResolution {
            ...
            if hasPerDevice {
                calibrationInfo += " (per-device data also saved)"
            }
        } else if hasPerDevice {
            calibrationInfo = "Per-device calibration active"
        } else {
            calibrationInfo = ""
        }
    }
...
struct AboutSettingsView: View {
    ...
    private let buildDate = "February 2026"
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Text("Control your Mac with touch input from supported touch displays")
...
struct WelcomePage: View {
    var body: some View {
        VStack(spacing: 20) {
            ...
            Text("Use your touch-enabled display to control your Mac with natural touch input.")
                .multilineTextAlignment(.center)
                .foregroundColor(.secondary)
...
struct CalibrationPage: View {
    var body: some View {
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                Label("Connect your touch display via USB-C", systemImage: "cable.connector")
